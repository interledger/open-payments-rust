...
impl<'a> AuthenticatedRequest<'a> {
    ...
    pub async fn build_request(self) -> Result<ReqwestRequest> {
        let mut builder = self
            .client
            .http_client
            .request(self.method.clone(), &self.url)
        ...
        *http_req.uri_mut() = req
        ...
        println!("Debug: Using key ID: {}", self.client.config.key_id);
        println!("Debug: Signature: {}", headers.signature);
        println!("Debug: Signature-Input: {}", headers.signature_input);
        ...
        println!("Full request headers: {:?}", req.headers());
