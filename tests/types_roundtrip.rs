...
fn access_item_roundtrip_variants() {
    let inc = AccessItem::IncomingPayment {
...
fn grant_and_continue_response_roundtrip_variants() {
    let cont = Continue {
        access_token: open_payments::types::ContinueAccessToken {
            value: "ctok".into(),
        },
        uri: "https://auth.interledger-test.dev/continue/abc".into(),
        wait: Some(1),
    };
    let with_token = GrantResponse::WithToken {
    ...
    let with_interaction = GrantResponse::WithInteraction {
        interact: InteractResponse {
            redirect: "https://auth.interledger-test.dev/interact/abc/finish".into(),
            finish: "finish-nonce".into(),
        },
        continue_: cont.clone(),
    };
    serde_roundtrip(&with_interaction);

    let cr_with_token = ContinueResponse::WithToken {
        access_token: AccessToken {
            value: "av".into(),
            manage: "https://auth.interledger-test.dev/manage".into(),
            expires_in: Some(3600),
...
fn receiver_walleturi_interval_roundtrip() {
    serde_roundtrip(&Receiver(
        "https://ilp.interledger-test.dev/incoming-payments/xyz".into(),
    ));
    serde_roundtrip(&WalletAddressUri(
        "https://ilp.interledger-test.dev/alice".into(),
    ));
...
fn grant_and_related_requests_roundtrip() {
    let at_req = AccessTokenRequest {
        access: vec![AccessItem::Quote {
            actions: vec![QuoteAction::Create],
        }],
    };
    ...
    let grant = GrantRequest::new(
        ...
        Some(InteractRequest {
            ...
            finish: Some(InteractFinish {
                ...
                uri: "https://client.example/finish".into(),
                nonce: "n".into(),
            }),
        }),
    );
    ...
    let ir = InteractResponse {
        redirect: "https://auth.interledger-test.dev/interact/abc/finish".into(),
        finish: "finish".into(),
    };
    serde_roundtrip(&ir);

    let cont_req = ContinueRequest {
        interact_ref: Some("ref123".into()),
    };
...
fn public_and_create_incoming_payment_roundtrip() {
    ...
    let cip = CreateIncomingPaymentRequest {
        wallet_address: "https://ilp.interledger-test.dev/alice".into(),
...
fn paginated_response_roundtrip() {
    ...
    let page = PaginatedResponse {
        pagination: PageInfo {
            start_cursor: Some("s".into()),
            end_cursor: Some("e".into()),
            has_next_page: false,
